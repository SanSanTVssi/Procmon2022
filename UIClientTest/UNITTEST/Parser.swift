...
func sleep_for (_ time_nanosec: Int32) {
    UIClient.sleep(time_nanosec);
}
...
class UIClientWrapper {
    ...
    static func getProcdata() -> SwiftProcdata {
        ...
        let to_return = SwiftProcdata.init(pid: pid, ppid: ppid, time: time, isOpen: isOpen, path: path, sha1Hash: hash, name: name)
